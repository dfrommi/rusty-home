...
| `grafanacloud-prom` | prometheus | **Infra + OTLP metrics** — `container_*`, `command_executed_total` (backend deliveries), `command_execution_total` (command outcomes), `home_state_calculation`, `z2m_state`, `traces_spanmetrics_*`. Free-tier retention. |
...
`container_*` comes from otel-collector's `docker_stats` receiver. `command_executed_total` (backend deliveries), `command_execution_total` (command outcomes), `home_state_calculation`, and `z2m_state` come from rusty-home's OTLP meter. `traces_spanmetrics_*` are auto-generated by Tempo.
...
# Command execution events for the Grafana command table
{service_name="rusty-home"} | event="command_executed"
...
**Command deduplication** (`should_execute` in `app/src/automation/planner/processor.rs`):

1. Same source and command executed less than 30s ago → skip while waiting for state feedback
2. Already reflected in device or notification state → skip ("nothing to do")

The last-execution record is maintained in memory by the automation planner. It is not read from command history or PostgreSQL. It is lost on restart, which allows commands to be retried after startup.
