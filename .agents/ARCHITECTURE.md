...
5. **Command execution** — `CommandClient` tries executors in order: Tasmota → Z2M → Nuki → HA. `is_reflected_in_state` checks and per-type cooldowns prevent redundant re-execution. Emits `CommandEvent::CommandExecuted`.
...
- **Executor fallback chain**: Tasmota → Z2M → Nuki → HA (first success wins).
