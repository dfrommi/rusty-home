...
use crate::{
    ...
    home::state::{AbsoluteHumidity, Temperature, sampled_data_frame},
...
impl DataPointAccess<DegreeCelsius> for FeltTemperature {
    async fn current_data_point(&self, api: &HomeApi) -> anyhow::Result<DataPoint<DegreeCelsius>> {
        ...
        let felt_temp_value = Self::calculate_felt_temperature(temp.value, abs_humidity.value);
...
mod tests {
    ...
    fn test_felt_temperature_hot_humid() {
        ...
        let felt_temp = FeltTemperature::calculate_felt_temperature(temp, abs_humidity);
